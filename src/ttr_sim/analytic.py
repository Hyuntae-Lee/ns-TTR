...
def analytic_surface_signal(r, t, laser: Laser, mat: Material, n_fine: int = 8192, n_quad: int = 48) -> np.ndarray:
    ...
    Gaussian pulse: Duhamel integral  dT(t) = f(0) S(t) + Int_0^t f'(t') S(t - t') dt'  evaluated as an
    FFT convolution on a fine uniform time grid (n_fine points over [0, max t]) and interpolated onto
    the requested times.  This is smooth (no staircase ripple) and costs a few hundred ms.
    """
    ...
    if laser.profile == "square":
        return (surface_step_response(r, t, q0, laser.w, mat, n_quad=64)
                - surface_step_response(r, t - laser.tau_p, q0, laser.w, mat, n_quad=64))
    t_end = float(np.max(t))
    tf = np.linspace(0.0, t_end, n_fine)
    dtf = tf[1] - tf[0]
    S = surface_step_response(r, tf, q0, laser.w, mat, n_quad=n_quad)          # (n_fine, nr)
    fp = laser.fprime(tf)                                                       # (n_fine,)
    conv = np.empty_like(S)
    for i in range(S.shape[1]):
        conv[:, i] = fftconvolve(fp, S[:, i])[:n_fine] * dtf
    Tf = float(laser.f(0.0)) * S + conv
    out = np.empty((len(t), len(r)))
    for i in range(len(r)):
        out[:, i] = np.interp(t, tf, Tf[:, i])
    return out
