...
해석해 비교를 GUI에서 바로 실행할 수 있습니다 (k_void 민감도 스윕은 테스트 코드에만 남아 있음).
...
* 관측 시간창: "관측 시간창 [μs]" 입력 하나. 프리셋을 바꾸면 대표 깊이 기준 기본값 max(5·τp, 4·d²/D) 이 다시 채워지고
  ...
  창을 넘으면 사이드바에 경고. 신호 탭의 시간축은 항상 선형이며 단위는 μs 로 고정.
...
* void: 축상 원판 또는 축대칭 링. 깊이·두께·반경 위치·반경 반폭은 μm 단위 숫자로 직접 입력 (깊이 프리셋을 바꾸면
  ...
  k_void 는 실제 공기값 0.026 W/m·K 로 고정 (GUI 표시만). 유한체적 정식화이므로 k_void→0 도 특이성 없어 바닥값(floor)이 필요 없음.
  §4의 4개 이산값 스윕(0.026 / 0.1 / 1 / 10)은 `validation.kvoid_sensitivity` 와 테스트 코드에만 남아 있으며,
  0.026 대비 0.1·1 은 1~5 % 이내, 10 은 약 8 % 차이임을 확인함.
